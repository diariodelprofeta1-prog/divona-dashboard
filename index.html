...
def generar_dashboard():
    ...
    for r in res.json().get("results", []):
        ...
        info = {"nombre": get_safe_text(p.get("Nombre")), "inicio": start, "fin": end, "estado": get_safe_text(p.get("Estado")), "tel": get_safe_text(p.get("Teléfono")), "dni": get_safe_text(p.get("DNI titular")), "coment": get_safe_text(p.get("COMENTARIOS"))}
        curr = start
        while curr <= end:
    ...
    <head>
        ...
        <style>
            ...
            .nav-link.active { border-bottom: 3px solid #3b82f6; color: #3b82f6; font-weight: 900; }
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-12 pb-8 border-b border-slate-800">
                <div class="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs uppercase tracking-widest text-slate-500 font-bold">Logbook & Fleet Command Center</p>
                    </div>
                </div>
            </div>
    ...
    for mes in range(3, 12):
        ...
        for dia in range(1, ultimo + 1):
            ...
            if res: html += f'<div onclick=\'alert("{res["nombre"]} - {res["estado"]}")\' class="day occupied">{dia}</div>'
            else: html += f'<div class="day free">{dia}</div>'
        html += "</div></div></div>"

    html += "</div></div></body></html>"
    # IMPORTANTE: Guardamos como index.html para la web
