...
def get_safe_text(prop):
    """Extrae el texto de forma segura sin importar el tipo de propiedad en Notion"""
...
def get_safe_num(prop):
    """Extrae números de forma segura (para el Precio)"""
...
def generar_dashboard():
    ...
    # FIJAMOS 6 FILAS PARA QUE NO SE CORTE NUNCA
    ...
    html += f'<p class="text-blue-400 font-bold text-xl mt-2">INGRESOS 2026: {total_anual:,.2f} €</p></div>'
    ...
    # Generar de Marzo a Diciembre (o el rango que prefieras)
    for mes in range(3, 13):
        ...
        for dia in range(1, ultimo + 1):
            ...
            click = f'onclick="alert(\'{ (res["nombre"] + "\\n" + res["detalle"] if res else "Día Libre") }\')"'
            
            html += f'<div class="{css}" data-tag="{tag}" style="{style}" {click}>'
    ...
    # Guardar archivo
    with open("dashboard_divona.html", "w", encoding="utf-8") as f:
        f.write(html)
    print("¡Hecho! Abre el archivo 'dashboard_divona.html' para ver el resultado.")
